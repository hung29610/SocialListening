  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-xl font-bold text-paper tracking-wide">API Keys & Webhooks</h2>
          <p className="text-sm text-paper-muted mt-1">Quản lý API keys để truy cập programmatic</p>
        </div>
        <button 
          onClick={() => {
            setShowModal(true);
      ...
      <div className="space-y-4">
        {apiKeys.length === 0 ? (
          <div className="text-center py-12 bg-void-surface border border-edge rounded-xl shadow-sm">
            ...
            <p className="text-paper-muted font-medium tracking-wide">Chưa có API key nào</p>
            <p className="text-sm text-paper-muted mt-1">Tạo API key để truy cập hệ thống qua API</p>
          </div>
        ) : (
          apiKeys.map((key) => (
              ...
              <div className="flex items-start justify-between">
                <div className="flex-1">
                  ...
                  <div className="mt-4 pt-4 border-t border-edge">
                    <p className="text-xs font-medium text-paper-muted mb-2.5">Quyền hạn ({key.permissions.length}):</p>
              ...
              <form onSubmit={handleCreate} className="p-6 space-y-5">
                <div>
                  <label className="block text-sm font-medium text-paper-muted mb-2">
                    Tên API Key <span className="text-destructive">*</span>
                  </label>
                  <input
                    type="text"
      ...
      <div className="bg-signal/10 border border-signal/25 rounded-xl p-4">
        <p className="text-sm text-paper-muted">
          <strong className="text-signal dark:text-signal-bright">Lưu ý:</strong> API keys cho phép truy cập programmatic vào hệ thống.
