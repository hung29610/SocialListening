          ...
          <div className="flex items-center space-x-4">
            ...
            <div>
              <div className="flex items-center space-x-3">
                <h2 className="text-xl font-bold text-paper tracking-wide">{t('crisis.warRoom.title')}</h2>
        ...
        <div className="flex-1 overflow-y-auto p-6">
          {loading ? (
            <div className="flex items-center justify-center h-64">
              <span className="text-paper-muted font-medium">{t('crisis.warRoom.settingUp')}</span>
            </div>
          ) : (
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
              ...
              <div className="lg:col-span-2 space-y-6">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div className={panel}>
                    <h4 className={`${eyebrow} mb-2`}>{t('crisis.warRoom.currentStatus')}</h4>
                    <span className="text-lg font-bold text-paper uppercase">{incident.status.replace('_', ' ')}</span>
                  </div>
                  <div className={panel}>
                    <h4 className={`${eyebrow} mb-2`}>{t('crisis.warRoom.deadline')}</h4>
                ...
                <div className={panel}>
                  ...
                  <div className="space-y-4">
                    <div className="relative border-l-2 border-signal/30 ml-3 pl-5 py-2">
                      ...
                      <p className="text-sm font-medium text-paper mt-1">{t('crisis.warRoom.incidentDetected')}</p>
                    </div>
                    {logs.map((log) => (
                ...
                {mention && mention.ai_analysis ? (
                  <div className={panel}>
                    ...
                    <div className="flex flex-col items-center justify-center p-4 bg-void-raised rounded-xl border border-edge mb-4">
                      ...
                      <span className={`${eyebrow} mt-1`}>{t('crisis.warRoom.riskScore')}</span>
                    </div>

                    <div className="space-y-3">
                      <div className="flex justify-between items-center pb-2 border-b border-edge">
                        <span className="text-xs text-paper-muted font-medium">{t('crisis.warRoom.crisisLevel')}</span>
                        <CrisisLevelBadge level={mention.ai_analysis.crisis_level} />
                      </div>
                      <div className="flex justify-between items-center pb-2 border-b border-edge">
                        <span className="text-xs text-paper-muted font-medium">{t('crisis.warRoom.sentiment')}</span>
                        <SentimentBadge sentiment={mention.ai_analysis.sentiment} size="sm" />
                      </div>
                      <div className="flex justify-between items-center pb-2 border-b border-edge">
                        <span className="text-xs text-paper-muted font-medium">{t('crisis.warRoom.urgency')}</span>
                  ...
                  <div className={`${panel} text-center`}>
                    <p className="text-sm text-paper-faint font-medium tracking-wide">{t('crisis.warRoom.noAiData')}</p>
                  </div>
                )}
