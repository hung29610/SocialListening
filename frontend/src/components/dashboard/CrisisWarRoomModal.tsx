...
export default function CrisisWarRoomModal({ isOpen, onClose, incident }: CrisisWarRoomModalProps) {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      toast.error(getErrorMessage(error) || t('crisis.warRoom.errors.loadFailed'));
    } finally {
      setLoading(false);
    }
  };
          ...
          <div className="flex items-center space-x-4">
            ...
            <div>
              <div className="flex items-center space-x-3">
                <h2 className="text-xl font-bold text-slate-900 dark:text-white tracking-wide">{t('crisis.warRoom.title')}</h2>
                <span className="px-2 py-0.5 text-xs font-bold uppercase tracking-wider rounded bg-gray-800 text-slate-500 dark:text-gray-400 border border-slate-300 dark:border-gray-700">
                  {t('crisis.warRoom.incidentNumber', { id: incident.id })}
                </span>
              </div>
        ...
        <div className="flex-1 overflow-y-auto p-6">
          {loading ? (
            <div className="flex items-center justify-center h-64">
              <span className="text-slate-500 dark:text-gray-400 font-medium">{t('crisis.warRoom.settingUp')}</span>
            </div>
          ) : (
              ...
              <div className="lg:col-span-2 space-y-6">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div className="bg-white dark:bg-[#111827] border border-slate-200 dark:border-gray-800 rounded-xl p-5">
                    <h4 className="text-[10px] font-bold text-gray-500 uppercase tracking-wider mb-2">{t('crisis.warRoom.currentStatus')}</h4>
                    <span className="text-lg font-bold text-slate-900 dark:text-white uppercase">{incident.status.replace('_', ' ')}</span>
                  </div>
                  <div className="bg-white dark:bg-[#111827] border border-slate-200 dark:border-gray-800 rounded-xl p-5">
                    <h4 className="text-[10px] font-bold text-gray-500 uppercase tracking-wider mb-2">{t('crisis.warRoom.deadline')}</h4>
                    <span className={`text-lg font-bold ${incident.is_overdue ? 'text-rose-500' : 'text-indigo-400'}`}>
                      {incident.deadline ? new Date(incident.deadline).toLocaleString('vi-VN') : t('crisis.warRoom.noDeadline')}
                    </span>
                  </div>
                </div>
                ...
                {incident.description && (
                  <div className="bg-white dark:bg-[#111827] border border-slate-200 dark:border-gray-800 rounded-xl p-5">
                    <h4 className="text-[10px] font-bold text-gray-500 uppercase tracking-wider mb-3 flex items-center">
                      ...
                      {t('crisis.warRoom.incidentDescription')}
                    </h4>
                ...
                <div className="bg-white dark:bg-[#111827] border border-slate-200 dark:border-gray-800 rounded-xl p-5">
                  <h4 className="text-[10px] font-bold text-gray-500 uppercase tracking-wider mb-4 flex items-center">
                    ...
                    {t('crisis.warRoom.eventTimeline')}
                  </h4>
                  <div className="space-y-4">
                    <div className="relative border-l-2 border-indigo-500/30 ml-3 pl-5 py-2">
                      ...
                      <p className="text-sm font-medium text-slate-900 dark:text-white mt-1">{t('crisis.warRoom.incidentDetected')}</p>
                    </div>
                    {logs.map((log) => (
                ...
                {mention && mention.ai_analysis ? (
                  <div className="bg-white dark:bg-[#111827] border border-slate-200 dark:border-gray-800 rounded-xl p-5">
                    <h4 className="text-[10px] font-bold text-gray-500 uppercase tracking-wider mb-4 flex items-center">
                      ...
                      {t('crisis.warRoom.originAnalysis')}
                    </h4>
                    ...
                    <div className="flex flex-col items-center justify-center p-4 bg-slate-50 dark:bg-[#0B1220] rounded-xl border border-slate-200 dark:border-gray-800 mb-4">
                      ...
                      <span className="text-[10px] font-bold text-gray-500 uppercase mt-1">{t('crisis.warRoom.riskScore')}</span>
                    </div>

                    <div className="space-y-3">
                      <div className="flex justify-between items-center pb-2 border-b border-gray-800/50">
                        <span className="text-xs text-slate-500 dark:text-gray-400 font-medium">{t('crisis.warRoom.crisisLevel')}</span>
                        <CrisisLevelBadge level={mention.ai_analysis.crisis_level} />
                      </div>
                      <div className="flex justify-between items-center pb-2 border-b border-gray-800/50">
                        <span className="text-xs text-slate-500 dark:text-gray-400 font-medium">{t('crisis.warRoom.sentiment')}</span>
                        <SentimentBadge sentiment={mention.ai_analysis.sentiment} />
                      </div>
                      <div className="flex justify-between items-center pb-2 border-b border-gray-800/50">
                        <span className="text-xs text-slate-500 dark:text-gray-400 font-medium">{t('crisis.warRoom.urgency')}</span>
                      ...
                      {mention.ai_analysis.escalation_needed && (
                        <div className="mt-4 p-2 bg-rose-500/10 border border-rose-500/20 rounded text-center">
                          <span className="text-xs font-bold text-rose-500 flex items-center justify-center">
                            ...
                            {t('crisis.warRoom.escalationRequired')}
                          </span>
                        </div>
                      )}
                    </div>
                  ...
                  <div className="bg-white dark:bg-[#111827] border border-slate-200 dark:border-gray-800 rounded-xl p-5 text-center">
                    <p className="text-sm text-gray-500 font-medium tracking-wide">{t('crisis.warRoom.noAiData')}</p>
                  </div>
                )}
                ...
                {mention && (
                  <div className="bg-white dark:bg-[#111827] border border-slate-200 dark:border-gray-800 rounded-xl p-5">
                    <h4 className="text-[10px] font-bold text-gray-500 uppercase tracking-wider mb-4 flex items-center">
                      ...
                      {t('crisis.warRoom.originalContent')}
                    </h4>
                    ...
                    >
                      <Link2 className="w-3.5 h-3.5 mr-1.5" /> {t('crisis.warRoom.viewAtSource')}
                    </a>)}
                  </div>
                )}
