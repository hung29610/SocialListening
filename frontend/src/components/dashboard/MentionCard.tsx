      ...
      <div className="px-4 py-3 bg-void-raised border-b border-edge flex flex-wrap items-center justify-between gap-2">
          ...
          <div className="flex flex-col">
            <div className="flex items-center gap-1.5">
              ...
              {typeof mention.source_confidence !== 'undefined' && !isLowConfidence && (
                <span title="Độ tin cậy cao"><ShieldCheck className="w-3.5 h-3.5 text-success" /></span>
              )}
              {isLowConfidence && (
                <span title="Độ tin cậy thấp"><ShieldAlert className="w-3.5 h-3.5 text-warning" /></span>
              )}
            </div>
        ...
        <div className="flex gap-1.5 items-center">
          {mention.ai_provider === 'failed' && (
            <span className="px-2 py-0.5 text-[9px] font-bold tracking-wider text-destructive bg-destructive/10 border border-destructive/25 rounded-md" title="AI Service Unavailable">
              AI FAILED
            </span>
          )}
      ...
      <div className="p-4 flex flex-col md:flex-row gap-4">
        ...
        <div className="flex-1 min-w-0 space-y-2">
          <h3 className="font-bold text-base text-paper leading-tight line-clamp-2">
            {mention.title || <span className="text-paper-faint italic font-normal">{t('mentions.noTitle') || "Không có tiêu đề"}</span>}
          </h3>
          <p className="text-sm text-paper-muted leading-relaxed line-clamp-3">
            {mention.content || <span className="text-paper-faint italic">{t('mentions.noContent') || "Không có nội dung trích xuất."}</span>}
          </p>
