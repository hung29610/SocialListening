      ...
      <div className="w-full max-w-2xl bg-void rounded-2xl shadow-tile border border-edge flex flex-col max-h-[90vh]">
        ...
        <div className="flex items-center justify-between p-6 border-b border-edge bg-void-surface rounded-t-2xl">
          <div className="flex items-center space-x-3">
            ...
            <div>
              <h2 className="text-lg font-bold text-paper tracking-wide">{t('crisis.evidence.title')}</h2>
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-6">
          <div className="bg-void-surface border border-info/30 rounded-xl p-4 flex items-start space-x-3">
            ...
            <div>
              <h4 className="text-sm font-bold text-info">{t('crisis.evidence.screenshotTitle')}</h4>
          ...
          <div className="flex items-center justify-between">
            <h3 className="text-sm font-bold text-paper tracking-wide uppercase">{t('crisis.evidence.listTitle')}</h3>
            {!showAdd && (
              <button
          ...
          {showAdd && (
            <div className="bg-void-surface border border-edge-strong p-5 rounded-xl space-y-4 relative overflow-hidden">
              ...
              <div>
                <label className="block text-eyebrow font-semibold text-paper-faint mb-1.5 uppercase">{t('crisis.evidence.sourceUrlLabel')}</label>
                <input
                  type="text"
                  value={url}
              ...
              <div>
                <label className="block text-eyebrow font-semibold text-paper-faint mb-1.5 uppercase">{t('crisis.evidence.noteLabel')}</label>
                <textarea
                  value={note}
          ...
          {loading ? (
            <div className="text-center py-10 text-paper-muted text-sm font-medium tracking-wide">{t('common.loading')}</div>
          ) : items.length === 0 ? (
            <div className="text-center py-12 bg-void-surface border border-edge rounded-xl border-dashed">
              ...
              <p className="text-sm text-paper-faint font-medium tracking-wide">{t('crisis.evidence.empty')}</p>
            </div>
          ) : (
