      ...
      <div className="w-full max-w-4xl bg-void rounded-2xl shadow-tile border border-edge overflow-hidden flex flex-col max-h-[90vh]">
        ...
        <div className="flex items-center justify-between p-6 border-b border-edge bg-void-surface">
          <div className="flex items-center space-x-3">
            ...
            <div>
              <h2 className="text-lg font-bold text-paper tracking-wide">{t('crisis.brief.title')}</h2>
              <p className="text-xs text-paper-muted mt-0.5">{t('crisis.brief.subtitle')}</p>
            </div>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-6">
          {loading ? (
            <div className="flex flex-col items-center justify-center py-20 space-y-4">
              ...
              <p className="text-paper-muted font-medium tracking-wide">{t('crisis.brief.generating')}</p>
            </div>
          ) : brief ? (
            <>
              {/* Meta Info */}
              <div className="flex flex-wrap gap-4 p-4 bg-void-surface border border-edge rounded-xl">
                <div className="flex flex-col">
                  <span className="text-[10px] uppercase font-bold text-paper-faint tracking-eyebrow mb-1">{t('crisis.brief.riskLevel')}</span>
                ...
                <div className="flex flex-col">
                  <span className="text-[10px] uppercase font-bold text-paper-faint tracking-eyebrow mb-1">{t('crisis.brief.recommendedAction')}</span>
                  <span className="text-sm font-semibold text-paper">{brief.recommended_decision}</span>
                </div>
                <div className="w-px h-10 bg-edge hidden sm:block"></div>
                <div className="flex flex-col">
                  <span className="text-[10px] uppercase font-bold text-paper-faint tracking-eyebrow mb-1">{t('crisis.brief.ownerDeadline')}</span>
              ...
              <div className="bg-void-surface border border-edge rounded-xl p-5">
                <div className="flex items-center justify-between mb-4">
                  ...
                  >
                    {copied === 'full' ? (
                      <><Check className="w-3.5 h-3.5 mr-1.5 text-success" /> {t('crisis.brief.copiedShort')}</>
                    ) : (
            ...
            <div className="text-center text-paper-faint py-10">{t('crisis.brief.noData')}</div>
          )}
        </div>
      </div>
    </div>
