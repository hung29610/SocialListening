    ...
    <AppCard
      ...
      header={
        <div className="flex justify-between items-center">
          <h2 className="text-base font-bold text-paper tracking-wide">{t('dashboard.panels.recentMentions')}</h2>
          <span className="text-[10px] font-bold tracking-eyebrow uppercase bg-signal/10 border border-signal/25 text-signal dark:text-signal-bright px-3 py-1.5 rounded-lg">Top 10</span>
        </div>
      }
    >
      <div className="h-full overflow-y-auto space-y-3 custom-scrollbar pr-2 -mr-2">
        {isLoading ? (
          ...
          <div className="flex flex-col items-center justify-center h-full text-paper-muted font-medium tracking-wide pb-10">
            ...
            <p className="text-sm text-paper-muted">{t('dashboard.panels.noRecentMentions')}</p>
            <p className="text-xs mt-1.5 text-paper-faint">{t('dashboard.panels.waitForData')}</p>
          </div>
        ) : (
