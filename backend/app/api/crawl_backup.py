                ...
                for mention_data in mentions:
                    ...
                    try:
                        ...
                        if "ai_provider_not_configured" in err_str or "openai_dependency_missing" in err_str or "API key is missing" in err_str or "not configured" in err_str:
                            msg = "AI chÆ°a cáº¥u hÃ¬nh, mention Ä‘Ã£ Ä‘Æ°á»£c lÆ°u nhÆ°ng chÆ°a phÃ¢n tÃ­ch AI."
                            prov = "skipped"
                        else:
                            msg = f"Lá»—i phÃ¢n tÃ­ch AI: {err_str}"
                            prov = "failed"
...
def get_capabilities():
    ...
    return {
        "web_search": {
            ...
            "message": None if has_serpapi else "ChÆ°a cáº¥u hÃ¬nh Web Search provider"
        },
        "auto_discovery": {
            "enabled": True,
            "configured": has_serpapi,
            "status": "READY" if has_serpapi else "CONFIG_REQUIRED",
            "message": None if has_serpapi else "ChÆ°a cáº¥u hÃ¬nh SerpAPI"
        }
    }
...
def debug_auto_discovery(
    ...
    has_serpapi = bool(settings.SERPAPI_API_KEY)
    if not has_serpapi:
        return {"success": False, "error": "ChÆ°a cáº¥u hÃ¬nh SERPAPI_API_KEY"}
...
def manual_scan(
    ...
    if not keyword_texts:
        raise HTTPException(status_code=400, detail="Vui lÃ²ng cung cáº¥p Ã­t nháº¥t má»™t tá»« khÃ³a (qua keyword_group_ids hoáº·c keywords)")
    ...
    if run_discovery:
        ...
        if not settings.SERPAPI_API_KEY:
            if mode == "AUTO_DISCOVERY":
                raise HTTPException(status_code=400, detail="CONFIG_REQUIRED: ChÆ°a cáº¥u hÃ¬nh Web Search provider (SerpAPI).")
            else:
                run_discovery = False
    ...
    if not sources_to_scan and mode == "SELECTED_SOURCES":
        raise HTTPException(status_code=400, detail="Pháº£i cung cáº¥p source_ids cho cháº¿ Ä‘á»™ SELECTED_SOURCES")

    if not sources_to_scan and not run_discovery and not body.url:
        raise HTTPException(status_code=404, detail="KhÃ´ng tÃ¬m tháº¥y nguá»“n hoáº¡t Ä‘á»™ng vÃ  khÃ´ng thá»ƒ cháº¡y Auto Discovery")
    ...
    msg_parts = []
    if sources_to_scan:
        msg_parts.append(f"quÃ©t {len(sources_to_scan)} nguá»“n")
    if run_discovery:
        msg_parts.append("tÃ¬m nguá»“n má»›i (SerpAPI)")
    ...
    return {
        ...
        "message": f"ÄÃ£ khá»Ÿi táº¡o scan: {' vÃ  '.join(msg_parts)}."
    }                
    ...
    try:
        ...
        if is_rss:
            ...
            if feed.bozo and not feed.entries:
                error_msg = str(feed.bozo_exception) if hasattr(feed, 'bozo_exception') else "XML khÃ´ng há»£p lá»‡"
                raise ValueError(f"rss_fetch_failed: Láº¥y dá»¯ liá»‡u RSS feed tháº¥t báº¡i: {error_msg}")
        ...
        # Try regular web scraping
            ...
            try:
                ...
                raise ValueError("timeout: Káº¿t ná»‘i háº¿t háº¡n (timeout). Vui lÃ²ng thá»­ láº¡i sau.")
            except Exception as e:
                raise ValueError(f"website_fetch_failed: Láº¥y dá»¯ liá»‡u trang web tháº¥t báº¡i: {str(e)}")
...
def retry_crawl_job(
    ...
    if job.status not in (CrawlJobStatus.FAILED, CrawlJobStatus.CANCELLED):
        raise HTTPException(status_code=400, detail="Chá»‰ cÃ³ thá»ƒ retry job Ä‘Ã£ tháº¥t báº¡i hoáº·c bá»‹ há»§y")
